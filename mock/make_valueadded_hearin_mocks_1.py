...
def main():
    ...
    satellites = np.where(GC['ID_host'] != -1)[0] #central galaxies (subhaloes)
    ...
    mock_halo_ids = HC['id'][host_haloes[mock_halo_inds]]
    print 'number of host haloes in mock:', len(mock_halo_ids)
    mock_subhalo_inds = np.in1d(HC['id'][subhaloes],GC['ID_halo'][satellites])
    mock_subhalo_ids = HC['id'][subhaloes[mock_subhalo_inds]]
    print 'number of subhaloes in mock:', len(mock_subhalo_ids)
    #find minimum halo mass
    min_halo_mass = np.min(HC['M200b'][mock_halo_inds])
    min_subhalo_Vpeak = np.min(HC['Vpeak'][mock_subhalo_inds])
    min_subhalo_Mpeak = np.min(HC['Mpeak'][mock_subhalo_inds])
    
    print 'minimum log halo mass:',          np.log10(min_halo_mass)
    print 'minimum sub halo peak velocity:', min_subhalo_Vpeak
    print 'minimum sub halo peak mass:',     min_subhalo_Mpeak

    mass_lim_haloes = np.where(((HC['M200b'] >= min_halo_mass) & (HC['pid']==-1)))[0] #host haloes to include
    print 'number of haloes >= min_mass:', len(mass_lim_haloes)
    halo_ids = HC['id'][mass_lim_haloes] #IDs of haloes >= minimum halo mass

    #idenitfy which haloes are above the minimum mass and not in the mock
    missing_halo_inds = np.in1d(halo_ids,mock_halo_ids)
    missing_halo_inds = (missing_halo_inds==False)
    missing_halo_ids = halo_ids[missing_halo_inds]
    missing_halo_inds = np.in1d(HC['id'],missing_halo_ids)
    print 'number of haloes to be added to mock:', len(missing_halo_ids)

    #identify the subhaloes belonging to the missing haloes
    missing_subhalo_inds = np.in1d(HC['pid'],missing_halo_ids) 
    #make cut to subhaloes
    #keep = np.where(HC['Vpeak'][missing_subhalo_inds]>=min_subhalo_Vpeak)[0]
    keep = np.where(HC['Mpeak'][missing_subhalo_inds] >= min_subhalo_Mpeak)[0]
    missing_subhalo_inds = missing_subhalo_inds[keep]
    missing_subhalo_ids = HC['id'][missing_subhalo_inds] #subhalo ids corresponding
    print 'number of subhaloes to be added to mock:', len(missing_subhalo_ids)
    ...
    N_entries = len(GC)+len(np.where(missing_halo_inds==True)[0])+len(np.where(missing_subhalo_inds==True)[0])
    ...
    #add empty haloes
    ind_begin = len(GC)
    ind_end   = len(GC) + len(np.where(missing_halo_inds == True)[0])
    ...
    #add empty subhaloes
    ind_begin = len(GC)+len(np.where(missing_halo_inds == True)[0])
    ind_end = len(GC)+len(np.where(missing_halo_inds == True)[0]) +\
              len(np.where(missing_subhalo_inds==True)[0])
    GC_new['ID_halo'][ind_begin:ind_end] = HC['id'][missing_subhalo_inds]
    GC_new['ID_host'][ind_begin:ind_end] = HC['pid'][missing_subhalo_inds]
    GC_new['x'][ind_begin:ind_end]       = HC['x'][missing_subhalo_inds]
    GC_new['y'][ind_begin:ind_end]       = HC['y'][missing_subhalo_inds]
    GC_new['z'][ind_begin:ind_end]       = HC['z'][missing_subhalo_inds]
    GC_new['Vx'][ind_begin:ind_end]      = HC['vx'][missing_subhalo_inds]
    GC_new['Vy'][ind_begin:ind_end]      = HC['vy'][missing_subhalo_inds]
    GC_new['Vz'][ind_begin:ind_end]      = HC['vz'][missing_subhalo_inds]
    GC_new['M_host'][ind_begin:ind_end]  = np.log10(HC['M200b'][missing_subhalo_inds])
    GC_new['M_vir'][ind_begin:ind_end]   = np.log10(HC['mvir'][missing_subhalo_inds])
