...
def main():
    ...
    for i in range(0,len(mass_bins)-1):
        ...
        if len(ind)>0: #if there are any haloes in the mass bin
            ...
            ran_index = np.random.random_integer(0,len(central__members)-1,len(satellite_members))
            ...
            for i in range(0,len(satellite_members)):
                ...
                new_host_ind = shuffled_central_members[new_host_ind]      #new host index
                #assign a new host properties
                GC_new['ID_host'][satellite_members[i]] = GC['ID_host'][new_host_ind]
                ...
                GC_new['Vx'][satellite_members[i]] = GC_new['Vx'][satellite_members[i]] - x_old_cen + x_new_cen
                GC_new['Vy'][satellite_members[i]] = GC_new['Vy'][satellite_members[i]] - y_old_cen + y_new_cen
                GC_new['Vz'][satellite_members[i]] = GC_new['Vz'][satellite_members[i]] - z_old_cen + z_new_cen
    ...
    filename = catalogue+'_sys_empty_shuffle'
    print filename
