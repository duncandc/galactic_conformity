...
#  Shuffle central-satellite systems as a whole amongst ALL haloes in a mass bin,
#  such that haloes that were previously empty, may afterwords contain a galaxy system.
...
def main():
    ...
    f =  h5py.File(filepath+catalogue+'_extended'+'.hdf5', 'r')
    GC = f.get(catalogue)
    GC = np.array(GC) #read into memory to speed up code

    #since we are ignoring subhaloes here, we can remove empty subhaloes.
    #remove = ((GC['g-r']==-99) & (GC['ID_host']!=-1)) #empty subhaloes
    #keep   = (remove==False)
    #GC     = GC[keep]

    #make new catalogue and copy over values from original catalogue
    dtype = GC.dtype.descr
    for d in dtype: print d
    ...
    for i in range(0,len(mass_bins)-1):
        ...
        #check to make sure all haloes chosen are in the mass bin
        print 'check:',((GC['M_host'][centrals[ind]]<mass_bins[i+1])&(GC['M_host'][centrals[ind]]>mass_bins[i]))
        if len(ind)>0: #if there are any haloes in the mass bin
            ...
            #This is the same as the thing I have done above... not really necessary!
            ...
            print 'check:', (satellite_members==sat_galaxy_members),(central_members==cen_galaxy_members)
            ...
            #shuffle centrals --> leave halo props alone, change gal props (haloes get new centrals)
            GC_new['M_r,0.1'][central_members] = np.array(GC['M_r,0.1'][shuffled_central_members], copy=True)
            GC_new['g-r'][central_members]     = np.array(GC['g-r'][shuffled_central_members],     copy=True)
            GC_new['M_star'][central_members]  = np.array(GC['M_star'][shuffled_central_members],  copy=True)
            #shuffle satellite systems --> leave gal props alone, change halo props (satellites get new haloes)
            for i in range(0,len(satellite_members)):
                ...
                new_host_ind = np.where(shuffled_central_members==old_host_ind)[0] #location in central members list
                new_host_ind = central_members[new_host_ind]              #new host index
                #print 'number of new hosts:', len(new_host_ind), old_host_ID, old_host_ind
                #assign new host properties
    ...
    catalogue = catalogue + '_sys_empty_shuffle'
